...
bool link_with_ld(const char *obj_filename, const char *exe_filename) {
    ...
    ld_args[arg_count++] = "/usr/lib/gcc/x86_64-linux-gnu/11/crtbegin.o"; // GCC runtime begin
...
bool get_gcc_file_path(const char *filename, char *buffer, size_t buffer_size) {
    ...
    if (fgets(buffer, buffer_size, fp) != NULL) {
        ...
        // Check if gcc actually found the file (it returns the filename unchanged if not found)
...
bool get_lib_paths(char *buffer, size_t buffer_size) {
    FILE *fp = popen("gcc -print-search-dirs | grep '^libraries:' | cut -d'=' -f2", "r");
    if (!fp) return false;
...
bool link_with_ld_simple(const char *obj_filename, const char *exe_filename) {
    ...
    bool found_crtbegin = get_gcc_file_path("crtbegin.o", crtbegin_path, sizeof(crtbegin_path));
    bool found_crtend = get_gcc_file_path("crtend.o", crtend_path, sizeof(crtend_path));
    
    if (!found_crt1 || !found_crti || !found_crtn || !found_crtbegin || !found_crtend) {
        printf("Warning: Could not locate all startup files. Trying common paths...\n");
    ...
    printf("  crti.o: %s\n", crti_path);
    printf("  crtn.o: %s\n", crtn_path);
    if (found_crtbegin) printf("  crtbegin.o: %s\n", crtbegin_path);
    if (found_crtend) printf("  crtend.o: %s\n", crtend_path);
    ...
    if (found_crtbegin && found_crtend) {
        snprintf(command, sizeof(command), 
                 ...
                 "%s %s %s %s -lc %s %s -o %s",
                 crt1_path, crti_path, crtbegin_path, obj_filename, 
                 crtend_path, crtn_path, exe_filename);
    } else {
