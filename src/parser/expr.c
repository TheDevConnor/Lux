/**
 * @file expr.c
 * @brief Expression parsing implementation for the programming language compiler
 * 
 * This file contains implementations for parsing all types of expressions in the
 * programming language. It works in conjunction with the Pratt parser implementation
 * in parser.c to handle operator precedence and associativity correctly.
 * 
 ...
 * 
 * This function handles the most basic expressions that don't have any operators:
 * integer literals, floating-point literals, string literals, character literals,
 * boolean literals, and identifiers. It uses a lookup table to map token types
 * to literal types and handles the appropriate conversion and memory allocation
 * for each type.
 * 
 ...
 * @note Identifiers are handled separately and create identifier expression nodes
 ...
 * 
 * This function handles prefix unary operators such as negation (-), logical NOT (!),
 * unary plus (+), and prefix increment/decrement (++, --). It uses a lookup table
 * to map token types to unary operator types and recursively parses the operand
 * with appropriate precedence.
 * 
 * @param parser Pointer to the parser instance
 * 
 ...
 * @note The operand is parsed with BP_UNARY precedence to handle operator nesting
 * @note Supports both arithmetic unary operators (-, +) and logical operators (!)
 * @note Prefix increment/decrement operators are handled here (postfix in prefix_expr)
 * 
 ...
 * 
 * @note Parentheses don't create additional AST nodes - they only affect parsing order
 * @note The inner expression is parsed with BP_LOWEST to allow any expression inside
 * @note Both opening and closing parentheses are consumed and required
 * @note Nested parentheses are handled naturally through recursive expression parsing
 * 
 ...
 * It's called as part of the left denotation (led) process in the Pratt parser.
 * 
 * @param parser Pointer to the parser instance
 ...
 * 
 * @note This function is part of the Pratt parser's led (left denotation) mechanism
 * @note The left operand is already parsed by the time this function is called
 * @note The right operand is parsed with the current binding power to handle precedence
 * @note Operator associativity is handled by the binding power passed to parse_expr
 ...
 * @note Arguments are stored in a growable array and can be of any expression type
 ...
 * @note The function being called (left operand) can be any expression (identifier, member access, etc.)
 * 
 ...
 * @note The right operand is parsed with BP_ASSIGN precedence for right-associativity
 * @note Validation of assignment targets is typically done during semantic analysis
 * 
 ...
 * @param bp The current binding power context (unused in this function)
 * 
 * @return Pointer to the appropriate expression AST node (member, index, or unary),
 *         or NULL if parsing fails
 * 
 ...
 * @note Postfix increment/decrement create unary expressions with special operators
 ...
 * @note Each element can be any valid expression (literals, identifiers, function calls, etc.)
